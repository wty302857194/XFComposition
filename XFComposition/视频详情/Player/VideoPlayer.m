...
@interface VideoPlayer()<PlaybackControlsDelegate>
...
/** 播放器配置信息 */
@property (nonatomic, strong) PlayerConfiguration *playerConfiguration;
/** 视频播放控制面板 */
@property (nonatomic, strong) PlaybackControls *playbackControls;
/** 非全屏状态下播放器 superview */
...
/** 播放器的播放状态 */
@property (nonatomic, assign) VideoPlayerState playerState;
/** 是否结束播放 */
...
//        [self _replayVideo];
    }else
    {
        ...
        if (self.playerState == VideoPlayerStatePause) {
            self.playerState = VideoPlayerStatePlaying;
        }
    }
}
...
- (void)_pauseVideo
{
    ...
    if (self.playerState == VideoPlayerStatePlaying) {
        self.playerState = VideoPlayerStatePause;
    }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context
{
    if ([keyPath isEqualToString:@"loadedTimeRanges"]){
        ...
        if (self.playerItem.playbackBufferEmpty) {
            self.playerState = VideoPlayerStateBuffering;
    ...
    {
        // 当视频缓冲好时
        if (self.playerItem.playbackLikelyToKeepUp && self.playerState == VideoPlayerStateBuffering){
            self.playerState = VideoPlayerStatePlaying;
        }
    }
    else if ([keyPath isEqualToString:@"status"])
    {
        if (self.player.currentItem.status == AVPlayerStatusReadyToPlay) {
            ...
            self.playerState = VideoPlayerStateFailed;
        }
    }
}
 ...
 */
- (void)_setVideoGravity:(VideoGravity)videoGravity
{
 ...
 */
- (void)setPlayerState:(VideoPlayerState)playerState
{
    _playerState = playerState;
    switch (_playerState) {
        case VideoPlayerStateBuffering:
        {
...
- (void)doubleTapGesture
{
    if (_playerConfiguration.supportedDoubleTap) {
        ...
        else if (self.playerState == VideoPlayerStatePause)
        {
            [self _playVideo];
        }
    }

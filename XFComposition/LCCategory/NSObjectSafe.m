...
void SFLog(const char* file, const char* func, int line, NSString* fmt, ...)
{
    ...
    NSLog(@"\n崩溃⚠️⚠️警告%s|%s|%d|%@", file, func, line, [[NSString alloc] initWithFormat:fmt arguments:args]);
    va_end(args);
}
...
- (void) hookAddObserver:(NSObject *)observer forKeyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options context:(void *)context
{
    if (!observer || !keyPath.length) {
        //SFAssert(@"hookAddObserver invalid args: %@", self);
        return;
    }
    @try {
...
{
    if (!observer || !keyPath.length) {
        //SFAssert(@"hookRemoveObserver invalid args: %@", self);
        return;
    }
    @try {
...
+ (NSString*) hookStringWithUTF8String:(const char *)nullTerminatedCString
{
    ...
    //SFAssert(@"NSString invalid args hookStringWithUTF8String nil cstring");
    return nil;
}
+ (nullable instancetype) hookStringWithCString:(const char *)cString encoding:(NSStringEncoding)enc
{
    ...
    //SFAssert(@"NSString invalid args hookStringWithCString nil cstring");
    return nil;
}
- (nullable instancetype) hookInitWithCString:(const char *)nullTerminatedCString encoding:(NSStringEncoding)encoding
{
    ...
    //SFAssert(@"NSString invalid args hookInitWithCString nil cstring");
    return nil;
}
...
- (nullable instancetype) hookInitWithCString:(const char *)nullTerminatedCString encoding:(NSStringEncoding)encoding
{
    ...
    //SFAssert(@"NSMutableString invalid args hookInitWithCString nil cstring");
    return nil;
}
- (void) hookAppendString:(NSString *)aString
{
    if (aString){
        ...
        //SFAssert(@"NSMutableString invalid args hookAppendString:[%@]", aString);
    }
}
- (void) hookInsertString:(NSString *)aString atIndex:(NSUInteger)loc
{
    if (aString && loc <= self.length) {
        ...
        //SFAssert(@"NSMutableString invalid args hookInsertString:[%@] atIndex:[%@]", aString, @(loc));
    }
}
- (void) hookDeleteCharactersInRange:(NSRange)range
{
    if (range.location + range.length <= self.length){
        ...
        //SFAssert(@"NSMutableString invalid args hookDeleteCharactersInRange:[%@]", NSStringFromRange(range));
    }
}
- (NSString *)hookStringByAppendingString:(NSString *)aString
{
...
+ (instancetype) hookArrayWithObject:(id)anObject
{
    ...
    //SFAssert(@"NSArray invalid args hookArrayWithObject:[%@]", anObject);
    return nil;
}
...
- (void) hookAddObject:(id)anObject {
    if (anObject) {
        ...
        //SFAssert(@"NSMutableArray invalid args hookAddObject:[%@]", anObject);
    }
}
- (id) hookObjectAtIndex:(NSUInteger)index {
    if (index < self.count) {
...
- (void) hookInsertObject:(id)anObject atIndex:(NSUInteger)index {
    if (anObject && index <= self.count) {
        ...
        if (!anObject) {
            //SFAssert(@"NSMutableArray invalid args hookInsertObject:[%@] atIndex:[%@]", anObject, @(index));
        }
        if (index > self.count) {
            //SFAssert(@"NSMutableArray hookInsertObject[%@] atIndex:[%@] out of bound:[%@]", anObject, @(index), @(self.count));
        }
    }
}
...
- (void) hookRemoveObjectAtIndex:(NSUInteger)index {
    if (index < self.count) {
        ...
        //SFAssert(@"NSMutableArray hookRemoveObjectAtIndex:[%@] out of bound:[%@]", @(index), @(self.count));
    }
}
...
- (void) hookReplaceObjectAtIndex:(NSUInteger)index withObject:(id)anObject {
    if (index < self.count && anObject) {
        ...
        if (!anObject) {
            //SFAssert(@"NSMutableArray invalid args hookReplaceObjectAtIndex:[%@] withObject:[%@]", @(index), anObject);
        }
        if (index >= self.count) {
            //SFAssert(@"NSMutableArray hookReplaceObjectAtIndex:[%@] withObject:[%@] out of bound:[%@]", @(index), anObject, @(self.count));
        }
    }
}
...
- (void) hookRemoveObjectsInRange:(NSRange)range {
    if (range.location + range.length <= self.count) {
        ...
        //SFAssert(@"NSMutableArray invalid args hookRemoveObjectsInRange:[%@]", NSStringFromRange(range));
    }
}
...
+ (instancetype) hookDictionaryWithObject:(id)object forKey:(id)key
{
    ...
    //SFAssert(@"NSDictionary invalid args hookDictionaryWithObject:[%@] forKey:[%@]", object, key);
    return nil;
}
+ (instancetype) hookDictionaryWithObjects:(const id [])objects forKeys:(const id [])keys count:(NSUInteger)cnt
{
    ...
    for (NSInteger i = 0; i < cnt ; ++i) {
        if (keys[i] && objects[i]) {
            ...
            //SFAssert(@"NSDictionary invalid args hookDictionaryWithObject:[%@] forKey:[%@]", objects[i], keys[i]);
        }
    }
...
- (void) hookSetObject:(id)anObject forKey:(id)aKey {
    if (anObject && aKey) {
        ...
        //SFAssert(@"NSMutableDictionary invalid args hookSetObject:[%@] forKey:[%@]", anObject, aKey);
    }
}
...
- (void) hookRemoveObjectForKey:(id)aKey {
    if (aKey) {
        ...
        //SFAssert(@"NSMutableDictionary invalid args hookRemoveObjectForKey:[%@]", aKey);
    }
}
...
+ (instancetype)hookSetWithObject:(id)object
{
    ...
    //SFAssert(@"NSSet invalid args hookSetWithObject:[%@]", object);
    return nil;
}
@end
...
- (void) hookAddObject:(id)object {
    if (object) {
        ...
        //SFAssert(@"NSMutableSet invalid args hookAddObject[%@]", object);
    }
}
...
- (void) hookRemoveObject:(id)object {
    if (object) {
        ...
        //SFAssert(@"NSMutableSet invalid args hookRemoveObject[%@]", object);
    }
}
@end
...
+ (instancetype)hookOrderedSetWithObject:(id)object
{
    ...
    //SFAssert(@"NSOrderedSet invalid args hookOrderedSetWithObject:[%@]", object);
    return nil;
}
- (instancetype)hookInitWithObject:(id)object
{
    ...
    //SFAssert(@"NSOrderedSet invalid args hookInitWithObject:[%@]", object);
    return nil;
}
...
- (void)hookAddObject:(id)object {
    if (object) {
        ...
        //SFAssert(@"NSMutableOrderedSet invalid args hookAddObject:[%@]", object);
    }
}
- (void)hookInsertObject:(id)object atIndex:(NSUInteger)idx
{
    if (object && idx <= self.count) {
        ...
        //SFAssert(@"NSMutableOrderedSet invalid args hookInsertObject:[%@] atIndex:[%@]", object, @(idx));
    }
}
- (void)hookRemoveObjectAtIndex:(NSUInteger)idx
{
    if (idx < self.count){
        ...
        //SFAssert(@"NSMutableOrderedSet invalid args hookRemoveObjectAtIndex:[%@]", @(idx));
    }
}
- (void)hookReplaceObjectAtIndex:(NSUInteger)idx withObject:(id)object
{
    if (object && idx < self.count) {
        ...
        //SFAssert(@"NSMutableOrderedSet invalid args hookReplaceObjectAtIndex:[%@] withObject:[%@]", @(idx), object);
    }
}
@end
...
- (void) hookSetObject:(id)value forKey:(NSString*)aKey
{
    if (aKey) {
        ...
        //SFAssert(@"NSUserDefaults invalid args hookSetObject:[%@] forKey:[%@]", value, aKey);
    }
}
- (void) hookRemoveObjectForKey:(NSString*)aKey
{
    if (aKey) {
        ...
        //SFAssert(@"NSUserDefaults invalid args hookRemoveObjectForKey:[%@]", aKey);
    }
}
...
- (void)hookSetObject:(id)obj forKey:(id)key // 0 cost
{
    if (obj && key) {
        ...
        //SFAssert(@"NSCache invalid args hookSetObject:[%@] forKey:[%@]", obj, key);
    }
}
- (void)hookSetObject:(id)obj forKey:(id)key cost:(NSUInteger)g
{
    if (obj && key) {
        ...
        //SFAssert(@"NSCache invalid args hookSetObject:[%@] forKey:[%@] cost:[%@]", obj, key, @(g));
    }
}
@end
